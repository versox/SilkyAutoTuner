...
class AutoFile {
public:
	typedef std::pair<std::string, bool> Operation;
private:
	...
	std::string readWait();
	std::string readDriveStraight(bool redIncluded = false);
	std::string readShoot();
	std::string readSpinUp();
	std::string readGearFlaps(bool open);
	std::string readSilkyRotate();
public:
